...
def extract_lineage(sql_query:str):
    ...
    for statement in parsed.tokens:      
      if statement.ttype is DML and statement.value.upper() in ("SELECT", "INSERT", "UPDATE", "DELETE"):
          for token in statement.tokens:            
              if isinstance(token, IdentifierList):
                  for identifier in token.get_identifiers():
                      columns.append(identifier.get_name())
              elif isinstance(token, Identifier):
                  if token.value.upper() not in ("FROM", "WHERE", "GROUP BY", "ORDER BY", "HAVING", "SELECT", "INSERT", "UPDATE", "DELETE"): # added filter conditions
    ...
    return {"tables": list(set(tables)), "columns": list(set(columns))}  # Remove duplicates